...
第 ${page}/${totalPages} 页，输入"图鉴 ${attrName} -p 页码"查看其他页`);
    ...
    if (stars === "all") {
      ...
      for (let star = 1; star <= 5; star++) {
        ...
        result.textOutput.join("\n").match(/(\S+): (\d+)/g)?.forEach((match) => {
          ...
          attrMap.set(name2, (attrMap.get(name2) || 0) + parseInt(value));
        });
      ...
      const selected2 = baseAttributes.sort(() => Math.random() - 0.5).slice(0, selectCount2);
      ...
      for (let star = 1; star <= 5; star++) {
        ...
        results.push({
          star,
          attributes: selected2.map(([name2]) => {
            const value = currentAttributes.get(name2) || 0;
            return {
              name: name2,
              value: Math.ceil(value * multiplier2)
            };
          })
        });
      }
      const totalResult = selected2.reduce((acc, [name2]) => {
      ...
      const output = [
        "🔥 全星级精工模拟（真实星级数据） 🔥",
        `使用材料：${materials}`,
        `随机选择 ${selectCount2} 条属性 x${multiplier2}`,
        "━━━━━━━━━━━━━━━━━━",
        ...results.map(
  ...
  ctx.command("营火签到").userFields(["authority"]).action(async ({ session }) => {
    ...
    const mainAttrResult = mainAttributes[equipment].reduce((acc, mainAttr) => {
      ...
      if (equipment === "头盔" && (mainAttr === "物抗" || mainAttr === "法抗")) {
        filteredAttributes = filteredAttributes.filter(
          ([name2]) => !["物抗", "法抗"].includes(name2)
        );
      } else {
        filteredAttributes = filteredAttributes.filter(
          ([name2]) => name2 !== mainAttr
        );
      }
